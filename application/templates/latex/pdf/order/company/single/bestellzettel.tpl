...
\begin{document}
	\vspace*{5mm}
        ...
        <</if>>
            ...
            <<__('Strasse:')|escape_latex>>                & \textbf{<<$order->getLocation()->getStreet()|escape_latex>> <<$order->getLocation()->getHausnr()|escape_latex>>}\\
            <<if !is_null($order->getLocation()->getOrt())>>
                <<__('Stadt:')|escape_latex>>                  & \textbf{<<$order->getLocation()->getPlz()|escape_latex>> <<$order->getLocation()->getOrt()->getOrt()|escape_latex>>}\\
            <<else>>
                <<__('Stadt:')|escape_latex>>                  & \textbf{<<__("k.A.")|escape_latex>>}\\
            <</if>>
            <<__('Lieferanweisungen:')|escape_latex>>      & \multicolumn{1}{v{13.5cm}}{\textbf{<<$order->getLocation()->getComment()|default:__("k.A.")|escape_latex>>}}\\
            <<if !is_null($order->getCustomer()->getCompany())>>
                <<__('Firma:')|escape_latex>>                  & \textbf{<<$order->getCustomer()->getCompany()->getName()|default:__("k.A.")|escape_latex>>}\\
            <<else>>
                <<__('Firma:')|escape_latex>>                  & \textbf{<<__("k.A.")|escape_latex>>}\\
            <</if>>
            <<__('Stockwerk:')|escape_latex>>              & \textbf{<<$order->getLocation()->getEtage()|default:__("k.A.")|escape_latex>>}\\
            <<__('Telefon:')|escape_latex>>                & \textbf{<<$order->getLocation()->getTel()|default:__("k.A.")|escape_latex>>}\\
        ...
        <<if $order->getService()->hasCourier()>>
            ...
            \textbf{<<__('Bestellung wird von %s abgeholt!', $order->getService()->getCourier()->getName()|escape_latex)|escape_latex>>}
            <<if $order->getService()->hasPromptCourier()>>
                ...
                <<__('Bei Verzögerungen oder evtl. Rückfragen erreichen Sie uns von 9.00 – 24.00 Uhr unter der folgenden Tel. Nummer: 030 609 88 548')|escape_latex>>
            <<else>>
                <<__('Lieber Restaurantmitarbeiter, in Kürze werden Sie einen Anruf von unserem Telefonsupport erhalten.  Wir würden Sie bitten die genaue Abholzeit mit unserem Supportmitarbeiter abzustimmen.')|escape_latex>>
                <<__('Damit für den Kurier keine Wartezeiten entstehen, bitten wir Sie den besprochenen Abholzeitpunkt genau einzuhalten. Für evtl. Rückfragen erreichen Sie uns von 9.00 – 24.00 Uhr unter der folgenden Tel. Nummer: 030 609 88 548')|escape_latex>>
            <</if>>
        <</if>>
		...
		\begin{longtable}{x{0.9cm}v{12cm}v{1cm}w{2cm}r}
            ...
            <<foreach from=$order->getCard() item=card>>
                <<foreach from=$card item=bucket>>
                    <<foreach from=$bucket item=cBucket>>
                        ...
                        <<assign var=mealoptioncount value=$meal->getCurrentMealOptionsCount()>>
                        <<assign var=mealoptionmaxprice value=$meal->getCurrentMealOptionsMaxPrice()>>
                        %item
                        ...
                        <<foreach from=$meal->getCurrentOptions() item=option>>
                            ...
                            <<if ($meal->getPriceType() == 'options_avg')>>
                            <<assign var=ocost value=$option->getCost()/$mealoptioncount>>
                            <<elseif ($meal->getPriceType() == 'options_max')>>
                            <<if $mealoptionmaxprice==$option->getCost()>>
                            <<assign var=ocost value=$mealoptionmaxprice>>
                            <<else>>
                            <<assign var=ocost value=0>>
                            <</if>>
                            <</if>>
                            <</if>>
                             <<assign var=mealTotal value=$mealTotal+$ocost>>
                            \textbf{<<$option->getName()|escape_latex>> <<if $ocost > 0>>(<<__('%s €', ($cBucket.count * $ocost)|inttoprice)|escape_latex>>),<</if>>}
                        <</foreach>>
            ...
            && \multicolumn{2}{r}{} & \\
            && \multicolumn{2}{r}{\textbf{<<__('Gesamtbetrag:')|escape_latex>>}} & \textbf{<<__('%s €', $order->getAbsTotal(false, false, true, false, false)|inttoprice)|escape_latex>>} \\
