 ...
 * 
 * based on @author Tudor Barbu <miau@motane.lu>
 * @author Matthias Laug <laug@lieferando.de> altered to use with classes and methods
 * @copyright MIT
 */
class Yourdelivery_Cronjob_Thread {
    ...
    private $errors = array(
        Yourdelivery_Cronjob_Thread::FUNCTION_NOT_CALLABLE => 'You must specify a valid function name that can be called from the current scope.',
        Yourdelivery_Cronjob_Thread::COULD_NOT_FORK => 'pcntl_fork() returned a status of -1. No new process was created',
    );
    ...
    public function setRunnable($_runnable) {
        if (self::runnableOk($_runnable)) {
            ...
            throw new Exception($this->getError(Yourdelivery_Cronjob_Thread::FUNCTION_NOT_CALLABLE), Yourdelivery_Cronjob_Thread::FUNCTION_NOT_CALLABLE);
        }
    }
    ...
    public function runnableOk($_runnable) {
        return method_exists($this, $_runnable);
    }
    ...
    public function start() {
        $pid = @ pcntl_fork();
        if ($pid == -1) {
            throw new Exception($this->getError(Yourdelivery_Cronjob_Thread::COULD_NOT_FORK), Yourdelivery_Cronjob_Thread::COULD_NOT_FORK);
        }
        if ($pid) {
            // parent 
            $this->pid = $pid;
            ...
            if (!empty($arguments)) {
                call_user_method($this->runnable, $this, $arguments);
            } else {
                call_user_method($this->runnable, $this);
            }
    ...
    public function stop($_signal = SIGKILL, $_wait = false) {
        if ($this->isAlive()) {
            ...
            if ($_wait) {
                return pcntl_waitpid($this->pid, $status = 0) > 0;
            }
        }
        return false;
    }
