...
abstract class Bvb_Grid {
            ...
            foreach ($filters as $key => $filter) {
                ...
                if (!is_array($filter) && (strlen($filter) == 0 || !in_array($key, $this->_fields))) {
                    ...
                    if (isset($this->_filters[$key]['callback']) && is_array($this->_filters[$key]['callback'])) {
                        ...
                        /**
                         * we can set a flag "nativeFilter" to deactivate the filters used by the grid
                         * @see http://ticket/browse/YD-2946
                         * @author Matthias Laug <laug@lieferando.de>
                         * @since 03.08.2012
                         */
                        
                        //check for nativeFilter Flag :)
                        if ( !isset($this->_data['fields'][$key]['nativeFilter']) || $this->_data['fields'][$key]['nativeFilter'] !== false ){                       
    ...
    public function getUrl($situation = '', $allowAjax = true)
    {
        ...
        foreach ($paramsClean as $key => $param) {
            ...
            // HACK! urlencode should be used for URL fragments escaping rather than htmlspecialchars
            $url .= sprintf('/%s/%s', urlencode($key), urlencode($param));
        }
    ...
    public function getHeaderScript(){
        if (method_exists($this, '_printScript') ){
