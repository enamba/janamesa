        ...
        if (count($this->_filters) > 0) {
            ...
            if ($this->getParam('order') && !$this->getParam('noOrder') && count($this->_filtersValues) > 0) {
                if ($this->getInfo("ajax") !== false) {
                    ...
                    $final1 = "<input type='button' onclick=\"window.location='$url'\" value='"
                            . $this->__('Remove Filters') . "'/><input type='button' onclick=\"window.location='$url2'\" value='"
                            . $this->__('Remove Order') . "'><input type='button' onclick=\"window.location='$url3'\" value='"
                            . $this->__('Remove Filters and Order') . "'>";
                }
                //Only filters
            } elseif ((!$this->getParam('order') || $this->getParam('noOrder')) && count($this->_filtersValues) > 0) {
                if ($this->getInfo("ajax") !== false) {
                    $final1 = "<input type='button' onclick=\"gridAjax('{$this->getInfo("ajax")}','" . $url . "') \">/";
                } else {
                    $final1 = "<input type='button' onclick=\"window.location='$url'\" value='" . $this->__('Remove Filters') . "'>";
                }
                ...
                if ($this->getInfo("ajax") !== false) {
                    ...
                    $final1 = "<input type='button' onclick=\"window.location='$url2'\" value='" . $this->__('Remove Order') . "'>";
                }
            }
            ...
            if (($this->getInfo('noFilters') != 1 || $this->_allowAdd == 1)
                ...
                if (strlen($final1) > 5 || $this->getUseKeyEventsOnFilters() == false) {
                    if ($this->getUseKeyEventsOnFilters() === false && $this->getInfo('noFilters') != 1) {
                        $final1 .= "<input type='button' onclick=\"_" . $this->getGridId() . "gridChangeFilters(1)\" value=\""  . $this->__('Apply Filter') . "\"/>";
                    }
...
function _" . $this->getGridId() . "gridChangeFilters(event)
    ...
    protected function _formatField($field)
    {
         ...
         * add hidden fields
         * 
         * @see http://ticket/browse/YD-2897
        ...
        foreach($params as $param => $value){
            $key = substr($param, 0, (-1) * strlen($this->_gridId));
            if ( $this->_data['fields'][$key]['hidden'] ){
                if ( !in_array($param, $duplicate) && preg_match(sprintf('/%s$/',$this->_gridId), $param) ){
                    $duplicate[] = $param;
                    $url .= '/' . $this->getView()->escape($param) . '/' . $this->getView()->escape($value);
                }
            }
        }
