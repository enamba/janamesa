...
if (file_exists('../storage')) {
    define('PATH_TO_STORAGE', '../storage/');
} elseif (file_Exists('storage')) {
    define('PATH_TO_STORAGE', './storage/');
} else {
    display_error('could not find storage');
}
...
/**
 * this rewrite rule must be implemented
 * RewriteRule ^service-image/(.*)/(.*).jpg$ /timthumb.php?serviceId=$1&name=$2 [L]
 */
if (isset($_GET['serviceId']) && !isset($_GET['mealId'])) {

    $src = '';
    if (isset($_GET['email'])) {
        // get modified size for picture in email
        $_GET['w'] = 265;
        $_GET['h'] = 173;
        $src = sprintf(PATH_TO_STORAGE . "restaurants/%d/default.jpg", (integer) $_GET['serviceId']);
    } elseif (isset($_GET['rating'])) {
        // get modified size for picture in rating sidebar
        $_GET['w'] = 54;
        $src = sprintf(PATH_TO_STORAGE . "restaurants/%d/default.jpg", (integer) $_GET['serviceId']);
        ...
        $serviceToWidth = 155;
        $src = sprintf(PATH_TO_STORAGE . "restaurants/%d/default.jpg", (integer) $_GET['serviceId']);
        $_GET['w'] = $serviceToWidth;
        $_GET['h'] = $serviceToHeight;
    }

}

/**
 * lighttpd rewrites in global config file on static2
 * 
 * url.rewrite-once = (
 *   "^/service-image/(.*)/(.*)" => "/timthumb.php?serviceId=$1&name=$2",
 *   "^/category-image/(.*)/(.*)" => "/timthumb.php?categoryId=$1&name=$2",
 *   "^/meal-image/(.*)/(.*)/(.*)" => "/timthumb.php?mealId=$1&serviceId=$2&name=$3"
 * )
 * 
 */
/**
 * this rewrite rule must be implemented
 * RewriteRule ^category-image/(.*)/(.*).jpg$ /timthumb.php?categoryId=$1&name=$2 [L]
 */
if (isset($_GET['categoryId'])) {
    // lets modify here
    $categoryToWidth = 700;
    $src = sprintf(PATH_TO_STORAGE . "category/%d-image.jpg", (integer) $_GET['categoryId']);
    $_GET['w'] = $categoryToWidth;
    //$_GET['h'] = $categoryToHeight;
}

/**
 * this rewrite rule must be implemented
 * RewriteRule ^meal-image/(.*)/(.*)/(.*).jpg$ /timthumb.php?mealId=$1&serviceId=$2&name=$3 [L]
 */
if (isset($_GET['mealId']) && isset($_GET['serviceId'])) {

    // lets modify here
    if ( in_array($_GET['serviceId'], array(13747,13748,16259,16260,16261,16262,16263,16264,16265,16266,16267,16268,16269,16270,16271,16272,16274,16275,16276,16277,16278,16279,16280,16281,16283,16284)) ){      
        $mealToHeight = 300;
        $mealToWidth = 300;
    }
    else{
    ...
    $src = sprintf(PATH_TO_STORAGE . "restaurants/%d/meals/%d/default.jpg", (integer) $_GET['serviceId'], (integer) $_GET['mealId']);
    $_GET['w'] = $mealToWidth;
    $_GET['h'] = $mealToHeight;
}
...
if (isset($_GET['customerId'])) {
    ...
    $_GET['h'] = 280;
    $_GET['c'] = 1;
    $src = sprintf(PATH_TO_STORAGE . "customer/%d/%s.jpg", (integer) $_GET['customerId'], $_GET['name']);
}

if (isset($src) && file_exists($src)) {
    $_GET['src'] = $src;
} else {
    display_error('Could not find meal image');
}
...
function check_cache($mime_type) {
    ...
    if (CACHE_USE) {
        ...
        if (!file_exists(DIRECTORY_CACHE)) {
            ...
            mkdir(DIRECTORY_CACHE, 0777, true);
...
function get_cache_file($mime_type) {
    ...
    if (!$cache_file) {
        $fileinfo = pathinfo($_SERVER['REQUEST_URI']);
        $path = DIRECTORY_CACHE . '/' . $fileinfo['dirname'];
        if (!is_dir($path)) {
            mkdir($path, 0755, true);
        }

        $cache_file = $path . '/' . urlencode(get_request('name')) . '.jpg';
